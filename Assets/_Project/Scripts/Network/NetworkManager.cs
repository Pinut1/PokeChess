...
public class NetworkManager : MonoBehaviourPunCallbacks
{
    ...
    public bool TryGetSavedBattleRecovery(int currentRound, out BattleSnapshot snapshot, out int tick)
    {
        ...
        Debug.Log($"[Network][Rejoin] 저장된 전투 복구 자료 발견 — round={savedRound}, tick={tick}");
        return true;
    }
    ...
    private void ResyncAfterReconnect()
    {
        ...
        if (PhotonNetwork.CurrentRoom.CustomProperties.TryGetValue(ROUND_PROP_KEY, out object roundObj))
        {
            ...
            if (currentRound > _lastKnownRound)
            {
                ...
                int myBattleResult =
                    PhotonNetwork.LocalPlayer.CustomProperties.TryGetValue(BATTLE_RESULT_PROP_KEY, out object myResult) && myResult is int myResultInt
                        ? myResultInt
                        : RESULT_NOT_REPORTED;
                bool myResultAlreadyReported = myBattleResult != RESULT_NOT_REPORTED;

                Debug.Log($"[Network][Rejoin] 라운드 복구 판정 — round={_lastKnownRound}→{currentRound}, " +
                          $"BattleActive={battleActive}, BattleResult={myBattleResult}");
                ...
                else
                {
                    ...
                    if (myResultAlreadyReported)
                    {
                        ...
                        // 재접속 직후 화면이 아무 페이즈도 반영하지 못한 채 기본값 Lobby에 남는 UX 갭을
                        // 최소한으로 메운다(EnterPhase(Result) 재사용 — 부작용 없음, 상세는 메서드 주석).
                        if (GameManager.TryGet(out var gmResult) && gmResult.Phase != null)
                            gmResult.Phase.RestoreResultPhaseAfterReconnect(currentRound);
