...
public static class TradeMachinePanelLayoutTool
{
    ...
    private static void CreateLayout()
    {
        ...
        Debug.Log($"[TradeMachinePanelLayoutTool] '{PANEL_NAME}' 생성/배선 완료.\n" +
                  "창 위치는 씬에서 직접 옮기세요 — 도구를 다시 돌려도 위치는 건드리지 않습니다.");
    }
    ...
    private static TradeMachinePanelUI BuildPanel(Transform canvas)
    {
        ...
        // 화면 오른쪽 가운데를 기본 자리로 둔다 — 통신기 옆으로는 씬에서 직접 옮길 것.
        SetRect(panel,
            new Vector2(1f, 0.5f), new Vector2(1f, 0.5f), new Vector2(1f, 0.5f),
            new Vector2(-40f, 0f), new Vector2(PANEL_WIDTH, 0f));
        ...
        RectTransform slotRoot = BuildSlotRow(panel);
    ...
    private static RectTransform BuildSlotRow(RectTransform parent)
    {
        RectTransform row = EnsureChild(parent, "Evolve_Slot_Panel");

        var grid = EnsureComponent<GridLayoutGroup>(row.gameObject);
        Undo.RecordObject(grid, UNDO_NAME);
        ...
        if (row.Find("UnitSlot") == null)
        {
            ...
            slot.name = "UnitSlot";
            Undo.RegisterCreatedObjectUndo(slot, UNDO_NAME);
        }
    ...
    private static void WireSellZone(TradeMachinePanelUI panel)
    {
        ...
        foreach (SellZone zone in zones)
        {
            ...
            Undo.RecordObject(zone, UNDO_NAME);
