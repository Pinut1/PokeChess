...
- ✅ **유닛 아이콘 위치** — `_1`(250×250) 사각형을 **전 종** 얼굴 위치로 맞춰둠. 툴팁에 뜨는 69종뿐 아니라
  나머지도 미리 잡아둔 상태라, 다른 UI에서 작은 아이콘이 필요해지면 그대로 쓰면 된다.
- ✅ **패널 높이** — 배경 border 100 / 바닥값 200으로 확정. 더 낮추지 않기로 함.
- 🔴 **단계별 효과 문구** — 시트 원본이 아직 `defense 고정값 증가` 같은 임시값이다.
  **기획 확정되면 시트 → JSON → `PokeChess/Import Synergy JSON` 재실행**만 하면 툴팁에 그대로 반영된다.
  코드·프리팹은 손댈 것이 없다.
