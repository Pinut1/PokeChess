...
| 담당자 | 파트 | 주요 스크립트 |
|--------|------|--------------|
| 김영욱 | Core / Network | `GameManager` `Singleton` `GameEvents` `NetworkManager` |
| 김기욱 | 전투 / 보드 | `BattleManager` `BoardManager` `SynergyManager` |
| 김태욱 | 상점 / 아이템 / UI | `ShopManager` `ItemManager` `UIManager` |

## 핵심 규칙
- **매니저끼리 직접 참조 금지** — 반드시 `GameEvents`를 통해 통신
- 새 이벤트는 `GameEvents.cs`에만 추가
- 다른 파트 매니저 건드릴 때는 담당자에게 먼저 확인
...
| `OnLevelChanged(int)` | ShopManager | UIManager |
