...
- [~] 🔺 **통신교환·통신진화 통합 대기** — 현재 검토 대상은 원격 브랜치 `origin/feature/trade-ui-integration`(최신 `0c315969`, 2026-07-27 확인)이다. 현재 통합 브랜치와 `origin/master`에는 아직 미병합. 구현 범위는 유닛 드래그 전송, 상대 대기열, 통신기 클릭 수령, 벤치 빈자리만큼 FIFO 일괄 수령, 성공 ACK, 장비·진화의 돌·통신진화 상태 이전, 전투 중 필드 유닛 진화 대기, 성급진화 연동이다. **재구현하지 말고** 현재 워킹 트리를 먼저 정리한 뒤 충돌 리뷰·병합·2클라 검증을 진행한다.
- [~] **통신기 정식 UI** — 최신 `0c315969`에서 `GameEvents.OnGoldTransferRequested(int)` 기반 골드 `1G/5G/10G` UI, 보유 골드별 버튼 활성화, ACK 완료·실패 표시까지 구현됐다. 골드는 버튼 클릭 즉시 전송하며 라운드당 횟수·총액 제한은 없다. 단, 골드 부족·파트너 미접속·기존 ACK 대기 중에는 거부한다. 브랜치 기록상 2인 테스트 완료이며, 현재 통합 브랜치와 `GameEvents`·`UIManager`·`NetworkManager`·씬이 크게 겹치므로 병합 후 전체 회귀 테스트가 필요하다.
...
**Supabase — 계정 공유 금지, 조직 구조에 따라 이전 방식 선택**
1. Supabase 전용 Organization이면 인수자를 **Owner**로 초대하고, 초대 수락·Play 검증 후 기존 Owner가 탈퇴한다.
2. 같은 Organization에 다른 프로젝트가 있으면 조직 Owner 초대는 다른 프로젝트까지 노출할 수 있다. 프로젝트 단위 역할은 Team/Enterprise에서만 제공되므로, 무료/Pro에서는 인수자의 별도 Organization으로 대상 프로젝트만 이전하는 방식을 우선 검토한다.
3. DB 비밀번호를 과거에 공유했다면 인수 완료 후 재설정한다. Unity 클라이언트는 DB 비밀번호를 사용하지 않는다.
4. Auth의 **Anonymous sign-ins 활성화** 상태를 유지한다. 꺼지면 전적은 로컬 jsonl에만 남는다.
5. `service_role`/secret key와 개인 Access Token은 Unity·Git·문서·메신저에 넣지 않는다.

세부 절차와 검증 체크리스트는 `Docs/HANDOFF_DAY1_2026-07-27.md`의 "Supabase 권한 인수인계"를 단일 기준으로 사용한다.
