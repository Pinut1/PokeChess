...
> 김영욱 조기퇴소(7/21)에 따른 최종 인수인계. 이 문서 기준 시점은 **2026-07-20, master `768adcc5`**.
...
| PR | 담당 | 처리 방안 |
|----|------|----------|
| **#40** `feat(shop): 2인 공용 상점 풀 동기화` | 태욱 | **닫기 권장.** #42(MasterClient 권위 방식)와 같은 기능의 경쟁 구현(PunRPC 방식)이었고, #42가 병합되며 대체됨. 상세 경위: `Docs/INVESTIGATION_2026-07-17_shared-pool-branches.md`. 태욱님이 직접 확인 후 닫고, `feature/shared-unit-pool-sync` 브랜치도 정리할 것. **그대로 병합하면 ShopManager/NetworkManager 대규모 충돌 발생하니 절대 병합 금지.** |
| **#46** `feat: 견본덱 UI` | 태욱 | 태욱님 작업. master 대상, 독립 브랜치. 리뷰 후 병합하면 됨. |
