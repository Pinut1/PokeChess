...
- [ ] 🔺 **통신교환 재설계 (설계 뒤집힘, 7/22 확인)** — 현재 코드는 모델A(1마리 파트너 핸드오버). 최종 방향은 **즉시진화 + 광역변환**(통신기에 올리면 즉시·비가역 진화, 실행 플레이어의 필드·대기석·**상점**이 진화체로 고정 = `affectsField/Bench/Shop`, 예: 스라크→핫삼 후 상점에도 핫삼만). 6/22에 기각했던 모델B 계열로 방향이 바뀜. **상세·미해결 기획질문(스탯 배율 3중 불일치·계산식 2중적용 함정·발동 성급·파트너 관여 여부)은 `NEXT_TASKS.md` 🔺 항목이 단일 기준.** 통신기 UI는 이 확정 전까지 홀드
- [ ] **통신기 정식 UI** — 골드 전송 백엔드는 완료. 유닛 통신진화는 위 재설계에 종속 → 모델 확정 후 착수. 현재 에디터 디버그 UI만 존재
- [x] ~~보스 전용 기믹~~ — **스코프 아웃(7/21)**: 기획에 보스 전용 패턴/페이즈 없음. 스탯 배수+스킬 구성이 최종 스펙
