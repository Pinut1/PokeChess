...
>
> **구현 상태**: CODEX가 2~5단계 구현(스키마/임포터, 유닛 영구 종교체·비주얼, 보드·벤치 일괄변환, 상점 override/원본풀 분리). 컴파일 0에러·에디터 진단 통과. → `feature/trade-evolution-redesign` (9032116b) 동결. **리뷰·잔여단계·병합 전.**
> - **잔여(미구현)**: 네트워크 발동 연결, Player CustomProperties 재접속 복구, Photon 2클라 회귀 테스트.
> - 상세 설계·구현 함정(공용 풀 ID/표시 ID 분리, `OnUnitChanged` 구독자 연결, 데이터 스왑 시 3D 모델 미교체 등): `Docs/INVESTIGATION_2026-07-22_trade-evolution-redesign.md`.
>
> **아직 열린 것 2가지 (병합 전 확인):**
